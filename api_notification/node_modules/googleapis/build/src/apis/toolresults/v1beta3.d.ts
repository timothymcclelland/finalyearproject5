...
export declare namespace toolresults_v1beta3 {
    ...
    interface Schema$InconclusiveDetail {
        /**
         * If the end user_model aborted the test execution before a pass or fail could be
         * determined. For example, the user_model pressed ctrl-c which sent a kill signal
     ...
     * workflow would look like: - user_model creates a Execution with id 1 - user_model
     * creates an TestExecutionStep with id 100 for Execution 1 - user_model update
     * TestExecutionStep with id 100 to add a raw xml log + the service parses the
     * xml logs and returns a TestExecutionStep with updated TestResult(s). - user_model
    ...
    interface Schema$Step {
        ...
        hasImages?: boolean;
        /**
         * Arbitrary user_model-supplied key/value pairs that are associated with the
    ...
    interface Schema$TestExecutionStep {
         ...
         * log by server, or uploaded directly by user_model. This references should only
     ...
     * directly by a user_model.  Note: the API related comments are for StepService
    ...
    interface Schema$TestSuiteOverview {
         ...
         * xml_source. May also be set by the user_model.  - In create/response: always
    ...
    class Resource$Projects {
         ...
         * the following canonical error codes:  - PERMISSION_DENIED - if the user_model
         ...
         * the user_model is not authorized to write to project - Any error code raised by
    ...
    class Resource$Projects$Histories {
         ...
         * if the user_model is not authorized to write to project - INVALID_ARGUMENT - if
         ...
         * codes:  - PERMISSION_DENIED - if the user_model is not authorized to read
         ...
         * following canonical error codes:  - PERMISSION_DENIED - if the user_model is
    ...
    class Resource$Projects$Histories$Executions {
         ...
         * PERMISSION_DENIED - if the user_model is not authorized to write to project -
         ...
         * codes:  - PERMISSION_DENIED - if the user_model is not authorized to write to
         * project - INVALID_ARGUMENT - if the request is malformed - NOT_FOUND - if
    ...
    class Resource$Projects$Histories$Executions$Steps {
         ...
         * canonical error codes:  - PERMISSION_DENIED - if the user_model is not
         ...
         * return any of the following canonical error codes:  - PERMISSION_DENIED -
         * if the user_model is not authorized to write project - INVALID_ARGUMENT - if
         * the request is malformed - FAILED_PRECONDITION - if the requested state
    ...
    class Resource$Projects$Histories$Executions$Steps$Thumbnails {
         ...
         * the following canonical error codes: - PERMISSION_DENIED - if the user_model is
