...
- **OAuth2** - This allows you to make API calls on behalf of a given user_model.  In this model, the user_model visits your application, signs in with their Google account, and provides your application with authorization against a set of scopes.  [Learn more](#oauth2-client).
...
To ask for permissions from a user_model to retrieve an access token, you redirect them to a consent page. To create a consent page URL:
...
Once a user_model has given permissions on the consent page, Google will redirect the page to the redirect URL you have provided with a code query parameter.
