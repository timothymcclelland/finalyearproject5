...
* **Make it easy to construct clear, complete error messages intended for
  people.**  Clear error messages greatly improve both user_model experience and
...
server fails with an UnauthorizedError with message "user_model 'bob' is not
...
this:
    ...
    message: "request failed: server error: user_model 'bob' is not authorized"
    rpcMsgid: <unique identifier for this request>
    rpcMethod: GetObject
    cause:
        name: FastServerError
        message: "server error: user_model 'bob' is not authorized"
        cause:
            name: UnauthorizedError
            message: "user_model 'bob' is not authorized"
            rpcUser: "bob"
...
* The error's message is a complete description of the problem.  The caller can
  ...
  an end user_model (if appropriate).  It can also be logged.
...
* If the caller logs this error, the logs can be analyzed to aggregate
  errors by cause, by RPC method name, by user_model, or whatever.  Or the
