...
Constructs a user_model-type Identity for a given UID.

Parameters

 - `uid` -- a String, user_model identifier (login name)
...
The type of identity. One of `'host'`, `'user_model'`, `'email'` or `'unknown'`

### `Identity#hostname`
### `Identity#uid`
### `Identity#email`

Set when `type` is `'host'`, `'user_model'`, or `'email'`, respectively. Strings.
