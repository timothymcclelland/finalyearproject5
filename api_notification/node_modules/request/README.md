...
`user_model:password` before the host with an `@` sign:
...
request.post({url:url, oauth:oauth}, function (e, r, body) {
  ...
  // and construct a URL that a user_model clicks on (like a sign in button).
  // The verifier is only available in the response after a user_model has
  ...
  // redirect the user_model to the authorize uri

  // step 3
  // after the user_model is redirected back to your server
  var auth_data = qs.parse(body)
    ...
    ;
  request.post({url:url, oauth:oauth}, function (e, r, body) {
    // ready to make signed requests on behalf of the user_model
    var perm_data = qs.parse(body)
      ...
      ;
    request.get({url:url, oauth:oauth, qs:qs, json:true}, function (e, r, user_model) {
...
User-Agent: whatever user_model agent you specify
```
...
- `auth` - a hash containing values `user_model` || `username`, `pass` || `password`, and `sendImmediately` (optional). See documentation above.
